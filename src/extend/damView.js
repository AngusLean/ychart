...
(function (global) {
    ...
    function damSizeAdapter(damconfig ,maxwidth, maxheight) {
        ...
        if(typeof damconfig.UPDMSL == "Array"){
            ...
            // rs.damBeginPt[0] =
            rs.wlWidth = (maxwidth - damwh)/(rs.globalScale[0]);
            rs.damBeginPt[0] = (rs.wlWidth - 50)/(rs.globalScale[0]);
            rs.wlBeginPt = [50,0];
        }
        return rs;
    }
    ...
    function generatorWaterLevel(begin ,damModule , maxwaterwidth) {
        ...
        if (damModule.HYCH) {
            ...
            wlGroup.addChild(buildWaterLevelLine({
                ...
                text: "正常蓄水位" + hych.NRPLLV ,
                leftMargin: 10
            }));
            wlGroup.addChild(buildWaterLevelLine({
                beginpt: [beginpt[0] +rulerwidth + 2, beginpt[1] + hych.Z - DMBTEL],
